'use strict';
app.controller('registerConfirmationController', ['$scope', '$routeParams', '$location', '$window', '$timeout', 'registrationService', 'ngAuthSettings',
    ...
    registrationService.getUserConfInfo($scope.regId).then(function (results) {
        $scope.userInfo = results.data;
		$scope.paymentUrl = results.data.paymentUrl;
        if ($scope.userInfo.hasPayment == false || $scope.paymentUrl != null) {
            //startTimer();
            $scope.loading = false;
        } else {
            //startTimerCheck();
    ...
    $scope.ActivateCheck = function () {
        $scope.loading = true;
        registrationService.getPaymentUrlConf($scope.regId).then(function (results) {
